...
-export([build/3, to_request/2, from_response/1, get_start_line_value/2, get_header_value/2]).
...
-spec to_request(RequestLine::list({atom(), string()}), Headers::list({string(), string()})) -> list(string()).
to_request(RequestLine, Headers) ->
  build_request_line(
    RequestLine,
